const App = {
  ...
  methods: {
    ...
    onValuesChange(e) {
      ...
      switch (id) {
        case "faculties":
          this.currentFloors = this.floors.filter(
            (floor) => floor.faculty == this.selectedFaculty
          )
          this.currentPulpits = this.pulpits.filter(
            (pulpit) => pulpit.faculty == this.selectedFaculty
          )
          break
        case "floors":
          break
